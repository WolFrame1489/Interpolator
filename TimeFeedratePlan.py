...
def planTime(times, spline, Movements: list, Amax, splineaxis): #осейвой сплайн нуджен чтобы знать длину вектора времени после выкидывания точек
    ...
    End = False
    i = 2
    dt = math.sqrt(sqeuclidean([spline[5][0], spline[5][1], spline[5][2]], [spline[6][0], spline[6][1], spline[6][2]])) / ((Movements[0].speed))
    #print(spline)
    ...
    while not End:
        if i < (len(splineaxis)):
            ...
            T[i] = T[i - 1] + dt
            print('T[i]', T[i], Currenttime + Movements[Counter].time, i)
